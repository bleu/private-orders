...
Next engineering action: run `./scripts/link-service-e2e.sh` and `./scripts/app-e2e.sh` against the offline stack once Docker is available, since that is the only evidence this revision has not reproduced — the real chain and a real extension wallet. After that, gate 3, agreeing the submission contract with CoW/BYOS, is the remaining decision rather than a remaining implementation.
