...
| Wallet and token support | Permit and approve funding both work, DAI-style permits are disclosed as unlimited, and a permit whose typed data cannot be produced falls back to `approve` instead of a `personal_sign` over a raw digest that no contract would accept. Open: contract-owner signatures are still restricted by local ECDSA/65-byte handling, and the supported wallet list is unnamed. | A deliberate smart-wallet/Safe story, named supported wallets and token contracts, and an extension-wallet run. See `page.mjs:421`, `:488`, `TokenPermit.sol:97`, `server.mjs:527`. |
| Browser trust | **Remediated locally.** Token symbols, wallet names, error strings, addresses and transaction hashes are escaped before they reach the DOM, and the browser flow was driven in real headless Chrome by `docs/review/page-safety.mjs`: a symbol containing an `<img onerror>` renders as text and runs nothing, a wrong chain disables signing, an approve-funded token sends exactly the calldata the server described, and an account switch is refused before a signature exists. `createChecked` still cannot protect a reader who signs arbitrary malicious calls from a compromised page. | An explicit threat model for a compromised page, and signed-payload validation beyond what the wrapper checks. See `page.mjs:133`, `server.mjs:446`. |
...
| Release proof | **Partly met.** Exact four-way deltas and no-residue assertions are in the link e2e; the flow has been run with a real wallet extension; every reproduced defect now has a regression test; and CI runs the contract, service and browser gates on every push plus the mainnet fork on demand. Open: independent contract review, staging approval, production configuration evidence, and a second supported wallet. See `scripts/link-service-e2e.sh`, `.github/workflows/test.yml`. | Independent audit/remediation, staging allowlisting, a named wallet support list, and a production configuration review. |
...
| `bash docs/review/run-counterexamples.sh` | Two contract regressions pass; two service counterexamples remain reproduced | Two contract regressions, three service regressions, 9 lifecycle checks, and 5 browser checks pass | Contract tests execute actual local GPv2 code. Service and page checks replace `forge`, `cast` and the orderbook at their boundaries and drive the real HTTP handler and the real page. |
| `./scripts/link-service-e2e.sh` against the offline stack | Not run: the stack was not up | Settled 100 USDC for 100 DAI; neither party sent a transaction; neither Shed gained; a second offer was cancelled on-chain and the cancellation retry converged | Real GPv2Settlement, real ComposableCoW, real COWShedFactory, real orderbook/autopilot/driver. Allowlisting is impersonated. |
| `./scripts/app-e2e.sh` against the offline stack | Not run: the stack was not up | The whole flow ran in two browser pages and settled at tx `0x863bce9d…`; maker received 100 DAI, taker received 100 USDC, neither party sent a transaction | Development wallets stand in for an extension; the signatures, relay, order and settlement are real. |
| The same flow with a **real wallet extension** (Rabby), driven by hand | Not run | Settled at tx `0x31d6c066…`: receipt status `0x1` **submitted by the relayer**, wrapper `consumed`, order `fulfilled`, and **neither party's nonce moved**. Both prompts rendered as readable EIP-712 — a COWShed domain and an `ExecuteHooks` message — and no prompt mentioned gas | One wallet, one chain, one pair. This is the check the development wallet cannot make. See `docs/WALLET.md`. |
| Exact four-way balances and residue | The app browser test asserted received balances *increased* | Each wallet moved by exactly the agreed amount (maker −100000000 USDC / +100000000000000000000 DAI, taker the mirror), and all four Shed balances were unchanged, so nothing was stranded | `scripts/link-service-e2e.sh` |

The contract cases pass only when the exploit attempts revert and protocol balances remain isolated. The service cases now assert the fixed behaviour instead of reproducing the defect: `docs/review/service-counterexamples.mjs` pins per-offer artifacts, evidence-based settlement and the relay's use of the accepted offer, and `docs/review/service-lifecycle.mjs` drives interleaving, two instances over one store, restart, duplicate and simultaneous acceptance, post-order failure with recovery, expiry, cancellation and the settlement evidence chain over real HTTP. `docs/review/page-safety.mjs` runs the real page in headless Chrome over the DevTools Protocol — no dependency, Node's own `WebSocket` — against the same fixtures. Every check in all three files was run against the pre-fix code and fails there.

One check was worthless when first written and only a mutation showed it: the unlimited-allowance disclosure was decided in two places on the server, so disabling one left the page still telling the truth by accident. The two were collapsed into one decision, and the check now fails when that decision is wrong.

The isolated counterexample build reports two unused-parameter warnings in `PrivateTradeBuilder.wrapperData`.

The offline orderbook and link service were not listening on ports 8080 and 9200 at review time, so neither e2e script was run then. Both were run afterwards against the offline stack (anvil on chain 1, plus orderbook, autopilot, driver and baseline) and both pass; the wallet-safety revision also added a cancellation leg to the link e2e, because cancellation was the one contract-level fix with no real-chain proof. The run found a real regression: the service stopped writing `out-json/link-computed.json` when per-offer artifacts were introduced, and `scripts/link-service-e2e.sh` still read the typed data from that file, so the check that a wallet is shown the same message as the digest was comparing the current digest against a two-day-old payload belonging to a different offer. It now reads the typed data from the service's role endpoint. The external Grok exploration lanes were unavailable and the configured Claude synthesis model returned an access/model error. Native agents recovered the transcript and checked documentation claims; this report does not claim a completed multi-provider security review.
...
4. Finish a restricted ERC20 desktop beta. Partly met: cancellation, recovery, text-only metadata rendering, an approve fallback, chain and account gating, and cancellation are implemented and tested. Open: exact wallet debits/credits proven on a real chain, a named-wallet extension run, readable decimal inputs, and a deliberate smart-wallet story. Open acceptance requires its own proof before enabling it.
...
Next engineering action: run the app flow once with a real wallet extension instead of the development wallet, since that is the last unproven step and the one where this project has already bled twice. After that, gate 3, agreeing the submission contract with CoW/BYOS, is the remaining decision rather than a remaining implementation.
